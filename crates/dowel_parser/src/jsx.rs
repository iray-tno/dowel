...
//!
//! Phase 0 scope only: static `className` string literals resolved through
//! the Tailwind table, `View`/`Text`/`Pressable`/`Button` primitives, plain
//! text children. Dynamic `className` expressions, event handlers, and
//! other props are not modeled yet -- unmapped attributes are silently
//! dropped for now rather than routed to `PropSet::passthrough`.
...
fn build_node(el: &JSXElement) -> Option<Node> {
    ...
    for attr_item in &el.opening_element.attributes {
        ...
        let Some(JSXAttributeValue::StringLiteral(literal)) = &attr.value else {
            continue; // dynamic className (ExpressionContainer): next pass
        };
    ...
    for child in &el.children {
        match child {
            JSXChild::Element(child_el) => {
                if let Some(child_node) = build_node(child_el) {
    ...
    Some(Node { primitive, style, props: PropSet::default(), children, text, span: to_span(el.span()) })
}
...
#[derive(Default)]
pub struct JsxCollector {
    pub roots: Vec<Node>,
}

impl<'a> Visit<'a> for JsxCollector {
    fn visit_jsx_element(&mut self, it: &JSXElement<'a>) {
        ...
        // a second time.
        if let Some(node) = build_node(it) {
